...
```{r}
#generate a sequence of all even numbers
evens <- seq(0, 1000, by = 2)
sum_evens <- 0

#sum all even numbers
for (even in evens) {
  sum_evens = sum_evens + even
}
sum_evens
```
...
```{r}
#add integers between 0 and 1000
sum(seq(0, 1000))
#add even integers between 0 and 1000
sum(seq(0, 1000, by = 2))
```
...
for (i in 1:nrow(survey.clean)) {
  for (j in 2:ncol(survey.clean)) {
    if (survey.clean[i, j] < 1 || survey.clean[i, j] > 5) {
      #sets dataframe to NA at row i and column j
      survey.clean[i, j] <- NA
    }
  }
}
...
for (i in 1:nrow(survey.clean)) {
  for (j in 1:ncol(survey.clean)) {
    #sums the number of NA values in each row and adds to column invalid.answers
    survey.clean$invalid.answers <- apply(survey.clean, 1, function(x) sum(is.na(x)))
  }
}
